...
#[test_only]
module turbos_clmm::swap_router_enhanced_simple_tests {
    use std::vector;
    ...
    fun setup_test_pool_with_liquidity(admin: address, user: address, scenario: &mut Scenario) {
        ...
        // Add liquidity to the pool using position_manager
        test_scenario::next_tx(scenario, admin);
        {
            ...
            // Mint large amount of tokens for liquidity
            ...
            // Use position_manager::mint to add liquidity, which correctly deposits tokens into the pool
    ...
    public fun test_swap_a_b_expired_deadline() {
        ...
        setup_test_pool_with_liquidity(ADMIN, USER, scenario);
    ...
    #[test]
    public fun test_swap_a_b_with_return_success() {
        let scenario_val = test_scenario::begin(ADMIN);
        let scenario = &mut scenario_val;
        ...
        test_scenario::next_tx(scenario, USER);
        {
            ...
            let safe_limit = current_price / 2; // Safe lower limit for a_to_b swap
    ...
    #[test]
    public fun test_swap_b_a_with_return_success() {
        let scenario_val = test_scenario::begin(ADMIN);
        let scenario = &mut scenario_val;
        ...
        test_scenario::next_tx(scenario, USER);
        {
            ...
            let current_price = pool::get_pool_sqrt_price(&pool);
            let safe_limit = current_price * 2; // Safe upper limit for b_to_a swap
    ...
    public fun test_swap_multiple_coins_merge() {
        ...
        test_scenario::next_tx(scenario, USER);
        {
            ...
            let current_price = pool::get_pool_sqrt_price(&pool);
            let safe_limit = current_price / 2;
    ...
    #[test]
    public fun test_swap_zero_coin_handling() {
        let scenario_val = test_scenario::begin(ADMIN);
        let scenario = &mut scenario_val;
    ...
    #[test]
    public fun test_swap_entry_functions() {
        let scenario_val = test_scenario::begin(ADMIN);
        let scenario = &mut scenario_val;
        ...
        {
            // Check if USER2 received USDC coins from the swap
            assert!(test_scenario::has_most_recent_for_address<Coin<USDC>>(USER2), 1);
        };
